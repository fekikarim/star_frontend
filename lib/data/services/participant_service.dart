...
class ParticipantService {
  ...
  Future<List<Participant>> getAllParticipants() async {
    try {
      ...
      final participants = response.map((json) => Participant.fromJson(json as Map<String, dynamic>)).toList();
  ...
  Future<Participant> createParticipant(CreateParticipantRequest request) async {
    try {
      AppLogger.info('Creating participant for challenge: ${request.challengeId}');
  ...
  /// Update participant
  Future<Participant> updateParticipant(String id, Map<String, dynamic> updates) async {
    try {
  ...
  Future<List<Participant>> getParticipantsByChallenge(String challengeId) async {
    try {
      AppLogger.info('Fetching participants for challenge: $challengeId');
      
      final allParticipants = await getAllParticipants();
      final challengeParticipants = allParticipants.where((p) => p.challengeId == challengeId).toList();
      
      AppLogger.info('Found ${challengeParticipants.length} participants for challenge: $challengeId');
      return challengeParticipants;
    } catch (e) {
      AppLogger.error('Failed to fetch participants for challenge: $challengeId', e);
      rethrow;
    }
  }
  ...
  Future<List<Participant>> getParticipantsByUser(String userId) async {
    try {
      ...
      final allParticipants = await getAllParticipants();
      final userParticipants = allParticipants.where((p) => p.utilisateurId == userId).toList();
      
      AppLogger.info('Found ${userParticipants.length} participations for user: $userId');
  ...
  Future<Participant> joinChallenge(String userId, String challengeId) async {
    try {
      ...
      AppLogger.info('User $userId successfully joined challenge: $challengeId');
      return participant;
    } catch (e) {
      AppLogger.error('Failed to join challenge: $challengeId for user: $userId', e);
      rethrow;
    }
  }
  ...
  Future<void> leaveChallenge(String userId, String challengeId) async {
    try {
      ...
      final participation = userParticipants.where((p) => p.challengeId == challengeId).firstOrNull;
      ...
      AppLogger.error('Failed to leave challenge: $challengeId for user: $userId', e);
      rethrow;
    }
  }
  
  /// Update participant score
  Future<Participant> updateParticipantScore(String participantId, double newScore) async {
    try {
      AppLogger.info('Updating score for participant: $participantId to $newScore');
      ...
      AppLogger.error('Failed to update score for participant: $participantId', e);
      rethrow;
    }
  }
  ...
  Future<bool> isUserParticipating(String userId, String challengeId) async {
    try {
      final userParticipants = await getParticipantsByUser(userId);
      final isParticipating = userParticipants.any((p) => p.challengeId == challengeId);
      
      AppLogger.info('User $userId participation in challenge $challengeId: $isParticipating');
      return isParticipating;
    } catch (e) {
      AppLogger.error('Failed to check participation for user: $userId in challenge: $challengeId', e);
      return false;
    }
  }
  
  /// Get user participation in challenge
  Future<Participant?> getUserParticipation(String userId, String challengeId) async {
    try {
      final userParticipants = await getParticipantsByUser(userId);
      final participation = userParticipants.where((p) => p.challengeId == challengeId).firstOrNull;
      
      AppLogger.info('Found participation for user $userId in challenge $challengeId: ${participation != null}');
      return participation;
    } catch (e) {
      AppLogger.error('Failed to get participation for user: $userId in challenge: $challengeId', e);
      return null;
    }
  }
  
  /// Get participant ranking in challenge
  Future<ParticipantRanking?> getParticipantRanking(String participantId) async {
    try {
      AppLogger.info('Getting ranking for participant: $participantId');
      
      final participant = await getParticipantById(participantId);
      final challengeParticipants = await getParticipantsByChallenge(participant.challengeId);
      
      // Sort by score descending
      challengeParticipants.sort((a, b) => b.scoreTotal.compareTo(a.scoreTotal));
      
      // Find rank
      final rank = challengeParticipants.indexWhere((p) => p.id == participantId) + 1;
      ...
      AppLogger.error('Failed to get ranking for participant: $participantId', e);
      return null;
    }
  }
  ...
  Future<Map<String, dynamic>> getParticipantStatistics(String participantId) async {
    try {
      ...
      final stats = {
        ...
        'scorePercentile': ranking != null 
          ? ((challengeParticipants.length - ranking.rank + 1) / challengeParticipants.length * 100).round()
          : 0,
      ...
      AppLogger.error('Failed to calculate statistics for participant: $participantId', e);
      rethrow;
    }
  }
}
