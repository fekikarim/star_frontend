...
class ChallengeService {
  ...
  Future<List<Challenge>> getAllChallenges() async {
    try {
      ...
      final challenges = response.map((json) => Challenge.fromJson(json as Map<String, dynamic>)).toList();
  ...
  /// Update challenge
  Future<Challenge> updateChallenge(String id, Map<String, dynamic> updates) async {
    try {
  ...
  Future<List<Participant>> getChallengeLeaderboard(String challengeId) async {
    try {
      ...
      AppLogger.info('Fetched leaderboard with ${participants.length} participants');
      return participants;
    } catch (e) {
      AppLogger.error('Failed to fetch leaderboard for challenge: $challengeId', e);
      rethrow;
    }
  }
  ...
  Future<List<Gagnant>> getChallengeWinners(String challengeId) async {
    try {
      ...
      final winners = response.map((json) => Gagnant.fromJson(json as Map<String, dynamic>)).toList();
      
      AppLogger.info('Fetched ${winners.length} winners for challenge: $challengeId');
      return winners;
    } catch (e) {
  ...
  Future<List<Challenge>> getActiveChallenges() async {
    try {
      ...
      final activeChallenges = allChallenges.where((challenge) => challenge.isActive).toList();
  ...
  Future<List<Challenge>> getCompletedChallenges() async {
    try {
      final allChallenges = await getAllChallenges();
      final completedChallenges = allChallenges.where((challenge) => challenge.isCompleted).toList();
      
      AppLogger.info('Found ${completedChallenges.length} completed challenges');
  ...
  Future<List<Challenge>> getUpcomingChallenges() async {
    try {
      final allChallenges = await getAllChallenges();
      final upcomingChallenges = allChallenges.where((challenge) => challenge.isUpcoming).toList();
  ...
  Future<List<Challenge>> getChallengesByStatus(String status) async {
    try {
      final allChallenges = await getAllChallenges();
      final filteredChallenges = allChallenges.where((challenge) => 
        challenge.statut.toLowerCase() == status.toLowerCase()).toList();
      
      AppLogger.info('Found ${filteredChallenges.length} challenges with status: $status');
  ...
  Future<List<Challenge>> getChallengesByCreator(String creatorId) async {
    try {
      final allChallenges = await getAllChallenges();
      final creatorChallenges = allChallenges.where((challenge) => 
        challenge.createurId == creatorId).toList();
      
      AppLogger.info('Found ${creatorChallenges.length} challenges created by: $creatorId');
  ...
  Future<List<Challenge>> searchChallenges(String query) async {
    try {
      final allChallenges = await getAllChallenges();
      final searchResults = allChallenges.where((challenge) => 
        challenge.nom.toLowerCase().contains(query.toLowerCase())).toList();
      
      AppLogger.info('Found ${searchResults.length} challenges matching: $query');
  ...
  Future<Map<String, dynamic>> getChallengeStatistics(String challengeId) async {
    try {
      ...
      final stats = {
        ...
        'averageScore': leaderboard.isNotEmpty 
          ? leaderboard.map((p) => p.scoreTotal).reduce((a, b) => a + b) / leaderboard.length 
          : 0.0,
        'topScore': leaderboard.isNotEmpty 
          ? leaderboard.map((p) => p.scoreTotal).reduce((a, b) => a > b ? a : b) 
          : 0.0,
